...
- Sidecar `ultron-memory.exe` (subcomandos: recall/stats/doctor/eval/reindex/reconcile/fts-rebuild/candidate/inbox…; sin argumentos lista todos. **OJO: `<sub> --help` no imprime ayuda, EJECUTA el subcomando**). Redeploy tras tocar Rust del sidecar: `node scripts/redeploy-memory-sidecar.mjs` (build con el target `ultron-memory`, shutdown/kill de los procesos, copia a `bin/`, verifica `version` == HEAD, relanza `serve`).
- **Dos índices derivados que se desincronizan en silencio** (2026-09-22): el FTS5 (`memory_items_fts`, contenido externo) llegó a 10.205 filas frente a 4.390 items porque `insert_item` escribe con `INSERT OR REPLACE` y el DELETE implícito de REPLACE no disparaba el trigger de borrado (cerrado con `PRAGMA recursive_triggers=ON` en `apply_schema`); el recall sparse devolvía basura sin que `integrity_check` de SQLite lo viera. Y una colección de Qdrant con el payload mmap corrupto (`value_start < mmap_len`) responde a `/collections` y a scroll sin payload, pero no con payload. `doctor` cubre ambos (checks `fts` y `reconcile`); reparación: `fts-rebuild` y borrar+`reindex` la colección (con reinicio controlado de Qdrant, ver papercuts).
...
- `ai_router/` (modulo; `route()` en `ai_router/providers/routing.rs`, re-exportada por `mod.rs`): `route(zone, prompt)` real con cadena primary→fallback. **Política CLI-first: las zonas de código arrancan por `codex-cli`** (suscripción ChatGPT, coste 0 por uso); el resto usa **groq como primary + gemini cloud como fallback** (gemini-cli se retiró el 2026-06-19: Google cortó el free-tier OAuth; sigue definido en `seed_providers` por si vuelve el tier, pero fuera de toda cadena). 6 zonas vivas, todas con llamante real (`research-web`, `routing-decision` y `code-fast-local` podadas el 2026-09-21; ollama queda como último relevo de `light`). Editar en `seed_zones()` + `cockpit/ai-router/zones.json`. Keys desde Settings → API Keys o `~/.ultron/.env` (dotenvy). Métrica honesta = `real_fallback_rate` (NO `attempt_failure_rate`). **OJO: el router NO rutea el Claude Code CLI** (esta sesión habla directo con Anthropic); solo afecta a las llamadas que hace la app Tauri.
- **Alcance real de las zonas de código (medido 2026-09-21)**: `code-edit` y `code-review` **no se han enrutado nunca** en producción — 21.773 rutas acumuladas en `metrics.json` solo registran groq y gemini, y las 5.305 líneas de `logs/route-decisions.jsonl` desde el 2026-08-27 son todas `zone: chat`. Su único llamante es `pick_analysis_zone()` (`library/ai_install.rs`), una acción manual de la pestaña Library. Configurarlas importa, pero no mueve la factura.
- **Invariante catálogo ↔ zonas (2026-09-21)**: entre el 2026-06-24 y hoy, el primary de las dos zonas de código fue `claude`, un id presente en `seed_providers()` que `load_providers()` **nunca copiaba** a un `providers.json` ya escrito (solo `migrate_stale_provider_models`, que repara modelos, no añade proveedores). El fichero tenía 8 proveedores y el seed 9. Cerrado por tres vías: `merge_missing_seed_providers` al cargar, `repair_zones_against_catalog` como última red (primary muerto → primer fallback vigente → modelo local), y tests en `ai_router_tests/basics.rs` que fallan si una zona del seed apunta a un proveedor inexistente o si una zona de código deja de arrancar por CLI. **La dedup de cadenas es por (proveedor, modelo), no por proveedor**: `chat` repite `groq` a propósito con dos modelos, porque la cuota de Groq es por modelo.
