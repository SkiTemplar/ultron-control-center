...
/// v15.4.9 — persistent user preference for the ULTRON mode. The session
/// file under `.tmp/` is regenerated by SessionStart hooks on every Claude
/// Code boot, which wiped any user override silently. The preferred-mode
/// file lives outside `.tmp/`, so it survives those regenerations. The
/// SessionStart hook can choose to seed `current-session.json` from this
/// preference; the Control Center UI reads/writes here as the source of
/// truth.
fn preferred_mode_file() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".ultron/cockpit/preferred-mode.txt"))
}
...
pub fn get_mode_inner() -> Result<ModeInfo, String> {
    ...
    // v15.4.9 — preferred-mode.txt wins over the ephemeral session file.
    // Fall back to the session JSON only if no preference is set yet.
    let preferred = preferred_mode_file()
        .and_then(|p| fs::read_to_string(&p).ok())
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty());
    let raw_mode = preferred.or_else(|| {
        raw.get("mode")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .or_else(|| {
...
pub fn reset_mode_to_autodetect_inner() -> Result<ModeSetResult, String> {
    ...
    // v15.4.9 — wipe the persistent preferred-mode file so autodetect
    // is the source of truth for the next session.
    if let Some(pref_path) = preferred_mode_file() {
...
pub fn set_mode_inner(payload: ModeSetPayload) -> Result<ModeSetResult, String> {
    ...
    // v15.4.9 — also persist the choice to the non-ephemeral preferred-mode
    // file so it survives SessionStart hook regenerations. This is the
    // file get_mode_inner reads first now.
    if let Some(pref_path) = preferred_mode_file() {
        if let Some(parent) = pref_path.parent() {
            let _ = fs::create_dir_all(parent);
        }
        let _ = fs::write(&pref_path, normalized.as_bytes());
    }
