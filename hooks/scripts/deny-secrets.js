 ...
 *
 * Instrumentacion (2026-09-22): era el UNICO de los 20 hooks registrados sin
 * una sola llamada a `observe`, y por eso tenia 0 registros en
 * hook-timing.jsonl frente a los 2.072 de codegraph-reminder, que comparte
 * parte del matcher. O sea: no habia forma de saber si el tripwire se habia
 * disparado alguna vez. Ahora cada ejecucion deja su linea de timing y las que
 * bloquean anotan ADEMAS `decision` (deny|ask), `rule` (el NOMBRE de la regla
 * que caso) y `tool`.
 *
 * Lo que NO se registra, a proposito: la ruta. Anotar "bloqueado
 * ~/.ssh/id_rsa_trabajo" en un log que la app lee y que puede acabar en un
 * repo publico convierte el guardian en la fuga (mandamiento 9). Los nombres
 * de regla salen de un conjunto cerrado escrito en este fichero, asi que no
 * hay dato del usuario que redactar.
 *
 * El fail-safe de este hook es distinto al del resto: los demas fallan
 * ABIERTOS (exit 0 sin salida ante cualquier excepcion) porque un hook roto no
 * puede tumbar todas las sesiones; este es una puerta de seguridad y falla
