...
| **Puntos de control** (deshacer lo que escribió un agente) | ✅ **nuevo** (2026-09-22) | Con una carpeta de proyecto fijada, antes de cada respuesta y de cada encargo se fotografía el árbol en un git en la sombra (`cockpit/maria/puntos/<hilo>.git`, sin tocar el `.git` del usuario). Cada respuesta ofrece «volver a antes de esta respuesta» con tres modos (solo el código / solo la conversación / las dos); `Alt+Z` o `/deshacer` vuelve al último punto; `/puntos` los lista. Si no se pudo fotografiar (árbol > 20.000 entradas, sin git) el turno lo dice. Barrido a los 30 días |
| Editor de atajos | ✅ **nuevo** (2026-09-22) | Ajustes → Atajos: cada acción con su combinación, «capturar» y «por defecto»; el backend rechaza duplicados y combos inválidos nombrando la fila; la app relee al guardar |
