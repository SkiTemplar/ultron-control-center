# ULTRON — advanced install reference

This file is the deep-dive reference: every wizard knob, every auto-installed
dependency, every step done by hand, every recovery path. **Reach for this
only when the bootstrap one-liner or `install.ps1` fail** — for the happy
path, read [`INSTALL.md`](../INSTALL.md) at the repo root.

Polished narrative walkthrough also lives at [`docs/INSTALL.md`](INSTALL.md).
...
The bootstrap installer does **not** remove anything. To roll back by hand:
...
- Bootstrap + manual installer fallback: `INSTALL.md`
